...
A `MapDescriptor` SHALL describe a fixed set of regions, each with a unique id, a point value, and
a rendering payload (an SVG path or equivalent), and an adjacency relation between region ids.
