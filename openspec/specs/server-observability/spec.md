...
## Purpose
The server logs room, connection, and player lifecycle events, rejected commands, and notable
in-game outcomes (elimination, base capture, game completion) at appropriate levels, so an operator
can follow what happened in a game from server log output alone, without attaching a debugger.
