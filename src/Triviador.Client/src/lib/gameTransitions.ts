...
export type GameTransition =
  ...
  | { kind: 'streakMilestone'; playerId: string; streak: number; tier: 1 | 2 | 3 }

// Fixed ladder of answer-streak values that produce a callout - lines up with PlayerRoster's own
// tier boundaries (bronze 1-3, silver 4-5, gold 6, rainbow 7+): 4 enters silver, 6 enters gold, 7
// enters rainbow. Streaks beyond the ladder keep escalating every +3 (10, 13, 16, ...) rather than
// going silent once the badge itself stops changing; tiers 1/2/3 map to the first three thresholds
// and every threshold past the third reuses tier 3's message.
const STREAK_MILESTONES = [4, 6, 7]
const STREAK_MILESTONE_STEP = 3

// Returns the highest milestone threshold crossed going from prevStreak to streak (streak assumed
// > prevStreak), or null if none was crossed - so a snapshot that jumps a player's streak by more
// than one still fires the highest threshold reached rather than none at all.
function crossedStreakMilestone(prevStreak: number, streak: number): { streak: number; tier: 1 | 2 | 3 } | null {
  const last = STREAK_MILESTONES[STREAK_MILESTONES.length - 1]
  for (let i = STREAK_MILESTONES.length - 1; i >= 0; i--) {
    const threshold = STREAK_MILESTONES[i]
    if (prevStreak < threshold && streak >= threshold) {
      return { streak, tier: (i + 1) as 1 | 2 | 3 }
    }
  }
  if (streak > last) {
    const prevSteps = Math.floor(Math.max(0, prevStreak - last) / STREAK_MILESTONE_STEP)
    const steps = Math.floor((streak - last) / STREAK_MILESTONE_STEP)
    if (steps > prevSteps) return { streak, tier: 3 }
  }
  return null
}
...
//
// A plain function (not a hook) so it can be called both from useGameTransitions (per-render, for
// toasts/sounds) and from gameStore's applyGameView (once per snapshot, to accumulate a whole-match
// recap log - see lib/recap.ts) without duplicating this derivation logic in two places.
export function deriveGameTransitions(current: GameView | null, previous: GameView | null): GameTransition[] {
  ...
  for (const p of current.players) {
    ...
    if (p.answerStreak > prevP.answerStreak) {
      const milestone = crossedStreakMilestone(prevP.answerStreak, p.answerStreak)
      if (milestone) {
        transitions.push({ kind: 'streakMilestone', playerId: p.playerId, streak: milestone.streak, tier: milestone.tier })
      }
    }
    if (
