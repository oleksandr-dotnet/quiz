...
export function AppShell({ topBar, map, dock, dockKey, roster, mapShaking, mapDanger }: AppShellProps) {
  const reducedMotion = usePrefersReducedMotion()
  const tone = dockToneFor(dockKey)
  const transition = reducedMotion ? { duration: 0 } : tone.transition
  const mapClassName = ['shell-map', mapShaking && 'shake', mapDanger && 'danger'].filter(Boolean).join(' ')
  return (
