...
export interface AppShellProps {
  ...
  mapDanger?: boolean
  // True while a correct/incorrect answer reveal is on screen - at phone width the map is hidden
  // entirely for that window (App.css's reveal-hide-mobile, mobile breakpoint only; a no-op on
  // desktop) rather than just shrunk, since a shrunk map was too small to read for the few seconds
  // the reveal is up and the room is better spent on the dock.
  mapRevealHidden?: boolean
}
...
export function AppShell({ topBar, map, dock, dockKey, roster, mapShaking, mapDanger, mapRevealHidden }: AppShellProps) {
  const reducedMotion = usePrefersReducedMotion()
  const tone = dockToneFor(dockKey)
  const transition = reducedMotion ? { duration: 0 } : tone.transition
  const mapClassName = ['shell-map', mapShaking && 'shake', mapDanger && 'danger', mapRevealHidden && 'reveal-hide-mobile']
    .filter(Boolean)
    .join(' ')
  return (
